...
IPv4Address::ptr IPv4Address::Create(const char* address, uint16_t port){
    ...
    int ret = inet_pton(AF_INET, address, &ipv4->m_addr.sin_addr);
    if(0 < ret)
        return ipv4;
    LOG_ERROR(g_logger) << "IPv4Address::Create(" << address << ", " << port    
            << ") rt=" << ret << " errno=" << errno << " errstr=" << strerror(errno);
    return nullptr;
}
...
IPAddress::ptr IPAddress::Create(const char* address, uint16_t port){
    ...
    if(error){
        LOG_INFO(g_logger) << "IPAddress::Create(" << address << ", "
...
IPv6Address::ptr IPv6Address::Create(const char* address, uint16_t port){
    ...
    int ret = inet_pton(AF_INET6, address, &ipv6->m_addr.sin6_addr);
    if(0 < ret)
        return ipv6;
