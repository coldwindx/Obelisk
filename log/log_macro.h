...
#define LOG_LEVEL(logger, level) \
	if(logger->getLevel() <= level) \
        obelisk::LogWriter(logger, obelisk::LogEvent::ptr(new obelisk::LogEvent(__FILE__, __LINE__, level, 0, thread_id(), fiber_id(), time(0)))).stream()
...
#define LOG_FMT_LEVEL(logger, level, fmt, ...) \
	if(logger->getLevel() <= level) \
		obelisk::LogWriter(logger, obelisk::LogEvent::ptr(new obelisk::LogEvent(__FILE__, __LINE__, level, 0, :thread_id(), fiber_id(), time(0)))).format(fmt, __VA_ARGS__)
